...
function gallery3_metabox_enqueue($hook) {
	if ( 'post.php' == $hook || 'post-new.php' == $hook ) {
		wp_enqueue_script('gallery3-metabox', get_template_directory_uri() . '/includes/mb3.js', array('jquery', 'jquery-ui-sortable'));
		wp_enqueue_style('gallery-metabox', get_template_directory_uri() . '/includes/mb.css');
	}
}
 ...
 * carousel 1 start
 */
add_action('admin_enqueue_scripts', 'gallery3_metabox_enqueue');
function add_gallery3_metabox($post_type) {
	$types = array('post', 'works');
	if (in_array($post_type, $types)) {
		add_meta_box(
			'gallery3-metabox',
			'Carousel 3',
			'gallery3_meta_callback',
			$post_type,
			'normal',
			'high'
		);
...
add_action('add_meta_boxes', 'add_gallery3_metabox');
function gallery3_meta_callback($post) {
	wp_nonce_field( basename(__FILE__), 'gallery3_meta_nonce' );
	$ids = get_post_meta($post->ID, 'vdw_gallery3_id', true);
	?>
	<table class="form-table">
		<tr><td>
				<a class="gallery3-add button" href="#" data-uploader-title="Add image(s) to gallery" data-uploader-button-text="Add image(s)">Add image(s)</a>

				<ul id="gallery3-metabox-list" class="gallery-metabox-list">
					<?php if ($ids) : foreach ($ids as $key => $value) : $image = wp_get_attachment_image_src($value); ?>

						<li style="float: left;width: 150px;">
							<input type="hidden" name="vdw_gallery3_id[<?php echo $key; ?>]" value="<?php echo $value; ?>">
							<img class="image-preview" src="<?php echo $image[0]; ?>">
							<a class="change-image button button-small" href="#" data-uploader-title="Change image" data-uploader-button-text="Change image">Change image</a><br>
							<small><a class="remove-image" href="#">Remove image</a></small>
						</li>
...
<?php }
function gallery3_meta_save($post_id) {
	if (!isset($_POST['gallery3_meta_nonce']) || !wp_verify_nonce($_POST['gallery3_meta_nonce'], basename(__FILE__))) return;
	if (!current_user_can('edit_post', $post_id)) return;
	if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) return;
	if(isset($_POST['vdw_gallery3_id'])) {
		update_post_meta($post_id, 'vdw_gallery3_id', $_POST['vdw_gallery3_id']);
	} else {
		delete_post_meta($post_id, 'vdw_gallery3_id');
	}
}
add_action('save_post', 'gallery3_meta_save');
